...
class DocumentProcessor:
    ...
    def __init__(self, embed_fn: Callable[[str], np.ndarray], chunk_size: int = 500, chunk_overlap: int = 50, log_fn: Callable[[str], None] = logging.info):
        """
        Args:
        ...
        for page_num in range(page_count):
            try:
                ...
                if not text.strip() and OCR_AVAILABLE:
                    self.log(f"⚠️ Page {page_num + 1} seems empty. Attempting OCR...")
                    try:
                        pix = page.get_pixmap()
                        img_data = pix.tobytes("png")
                        image = Image.open(io.BytesIO(img_data))
                        text = pytesseract.image_to_string(image)
                        self.log(f"✅ OCR extracted {len(text)} chars.")
                    except Exception as e:
                        self.log(f"❌ OCR failed for page {page_num + 1}: {e}")
                ...
                self.log(f"⚠️ Warning: Skipping page {page_num + 1} in {filename} due to error: {e}")
                continue
